...
async function main() {
  ...
  log("Cron pipeline result", {
    ...
    aiMaxItems: result?.aiClassification?.maxItems ?? null,
    aiConcurrency: result?.aiClassification?.concurrency ?? null,
    aiBatchSize: result?.aiClassification?.batchSize ?? null,
